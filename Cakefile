...
# cake docs
# ```
task 'docs', 'generate documentation', -> coffeedoc()
...
launch = (cmd, options=[], callback, spawn_opts = {}) ->
  cmd = which(cmd) if which
  app = spawn cmd, options, spawn_opts
...
mocha = (options, callback) ->
  ...
  launch './node_modules/.bin/mocha', options, callback, {env: _.extend {"NODE_PATH": "./lib"}, process.env}

# ## *coffeedoc*
#
# **given** optional function as callback
# **then** invoke launch passing coffeedoc command
coffeedoc = (callback) ->
  #if moduleExists('docco')
  walk 'src', (err, files) -> launch './node_modules/.bin/coffeedoc', ['--hide-private', files...], callback
